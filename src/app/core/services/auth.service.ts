...
export class AuthService {
  ...
  public async linkUserToEmployee(uid: string, employeeId: string | null): Promise<void> {
    if (this.currentUser()?.uid === uid) {
      this.currentUser.update(user => user ? { ...user, linkedEmployeeId: employeeId || undefined } : null);
    }
    this.allUsers.update(users =>
      users.map(u => (u.uid === uid ? { ...u, linkedEmployeeId: employeeId || undefined } : u))
    );
    try {
      const userRef = doc(this.firebase.db, 'users', uid);
      await setDoc(userRef, { linkedEmployeeId: employeeId || null }, { merge: true });
    } catch (e) {
      console.warn('Firestore link user fallback:', e);
    }
  }
