...
export class EmployeeFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.employeeForm = this.fb.group({
      ...
      // Detalles de Empleo
      employeeId: [autoId, Validators.required],
      linkedUserId: [''],
  ...
  private loadEmployeeData(id: string) {
    ...
    if (found) {
      this.employeeForm.patchValue({
        ...
        taxId: found.taxId || '',
        linkedUserId: found.linkedUserId || '',
  ...
  public async onSubmit() {
    ...
    try {
      let savedEmpId = this.employeeIdParam();
      if (this.isEditMode() && savedEmpId) {
        await this.empService.updateEmployee(savedEmpId, employeePayload);
      } else {
        savedEmpId = await this.empService.addEmployee(employeePayload);
      }

      if (employeePayload.linkedUserId) {
        await this.auth.linkUserToEmployee(employeePayload.linkedUserId, savedEmpId);
      }
