...
export class SettingsComponent {
  ...
  public async linkEmployeeProfile(uid: string, event: Event) {
    const select = event.target as HTMLSelectElement;
    const empId = select.value || null;

    await this.auth.linkUserToEmployee(uid, empId);
    this.toastNotice.set(`Vinculación de perfil actualizada en Firestore.`);
    setTimeout(() => this.toastNotice.set(null), 4000);
  }

  public getLinkedEmployeeName(linkedId?: string): string {
    if (!linkedId) return 'Usuario Maestro Pure Admin (Sin Ficha)';
    const emp = this.empService.employees().find(e => e.id === linkedId || e.employeeId === linkedId);
    return emp ? `${emp.fullName} (${emp.position})` : 'Ficha no encontrada';
  }
